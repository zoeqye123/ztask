...
- 用户说"做一个仪表盘" → 先问："你真正想监控的指标是什么？做决策时需要看到什么？"
- 用户说"优化这个流程" → 先问："当前流程哪里最慢？瓶颈在哪？优化多少才算值？"
- 用户说"集成某个工具" → 先问："你想解决什么能力缺口？有没有更简单的方案？"
- 不要把用户需求当作既成事实。从零思考：如果不考虑任何现有条件，解决这个问题的本质是什么？
...
- 不要套模板思维——这个任务的最简完成路径是什么？
- 交付物是否直接命中问题本质？有没有更少的步骤达成同样效果？
- 每个 Phase 的存在是否必要？去掉它，目标还能达成吗？
...
Task 生成后，站在"最苛刻的审查者"视角逐项回看：

- **假设每个步骤都会失败**：最可能在哪里出错？有没有兜底方案？
- **寻找模糊点**：验收标准有没有"看着都对了但实际没做到"的灰色空间？
- **挑战前提条件**：Task 依赖的外部条件（环境、权限、数据）真的成立吗？不成立怎么办？
- **对每个目标问"然后呢？"**：验收通过后，交付物真的能解决原始问题吗？

根据审视结果，补充边界条件、异常处理和验收标准的严格度。
